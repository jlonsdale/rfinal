  ...
  while ((line = lines[index++]) != null) {
    ...
    switch (command) {
      ...
      case "Kd": // Read diffuse color coefficient as color
        ...
        if (material) material.color = mtl.parseRGB(sp);
        name = "";
        continue; // Go to the next line
      case "Ka": // Read ambient color coefficient as emission
        if (material) material.emission = mtl.parseRGB(sp);
        name = "";
        continue; // Go to the next line
      case "Ks": // Read specular color coefficient
        if (material) material.specular = mtl.parseRGB(sp);
        name = "";
        continue; // Go to the next line
      case "Ni": // Read specular color coefficient
        if (material) material.ior = sp.getFloat();
        name = "";
        continue; // Go to the next line
      case "Ns": // Read specular color coefficient
        if (material) material.shininess = sp.getFloat();
        name = "";
        continue; // Go to the next line
      case "illum": // Read specular color coefficient
        if (material) material.illum = sp.getInt();
        name = "";
        continue; // Go to the next line
    }
  }
  mtl.complete = true;
  console.log("bingbong am done");
}
...
OBJDoc.prototype.getDrawingInfo = function () {
  ...
  for (var i = 0; i < this.objects.length; i++) {
    ...
    for (var j = 0; j < object.faces.length; j++) {
      ...
      var color =
        mat.color === undefined ? new Color(0.8, 0.8, 0.8, 1.0) : mat.color;
