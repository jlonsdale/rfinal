...
const compute_jitters = () => {
  jitterVectors = [];
  const jitterScale = 0.01; // Adjust this scale factor to control jitter size
  ...
  for (let i = 0; i < subdivisionLevel; i++) {
    for (let j = 0; j < subdivisionLevel; j++) {
      ...
      jitterVectors.push(jitterX, jitterY);
    }
  }
};

let jitterVectors = []; // Initialize an empty array to store jitter vectors
let subdivisionLevel = 1; // Initial subdivision level
let uniforms;
...
async function main() {
  ...
  const subdivisionLevelInput = document.getElementById("subdivisionLevel");

  // Add an event listener to handle changes in the subdivision level
  subdivisionLevelInput.addEventListener("input", () => {
    subdivisionLevel = parseInt(subdivisionLevelInput.value);
    const subdivisionValueSpan = document.getElementById("subdivisionValue");
    subdivisionValueSpan.textContent = subdivisionLevel;
    tick();
  });
  ...
  const tick = () => {
    ...
    const jitters = new Float32Array(jitterVectors);
    console.log(jitterVectors, jitters);
    ...
    const uniforms = new Uint32Array([subdivisionLevel]);
